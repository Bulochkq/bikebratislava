...
// Header setup.
// On the static site the header was fetched from header.html and injected at
// runtime. In the theme it is rendered server-side, so we only wire up the
// behaviour — no network request, and nothing breaks if the request fails.
function initHeader() {
    const header = document.getElementById('main-header');
    if (!header) return;
    ...
    // Highlight active link. WordPress serves clean URLs (/tours/), but this
    // also tolerates missing trailing slashes, .html requests and the site root.
    const segments = window.location.pathname.split('/').filter(Boolean);
    const currentPage = (segments.pop() || '').replace(/\.html$/, '');
    ...
    // Apply the correct header styling immediately (prevents a transparent
    // header when the page is loaded already scrolled down).
